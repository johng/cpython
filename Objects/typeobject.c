...
#endif
    if (can_cache) {
        /* Store the version first: the release-store of the pointer below
         * then publishes both, so a reader that acquire-loads a non-NULL
         * setitem cannot pair it with a version from a different function. */
